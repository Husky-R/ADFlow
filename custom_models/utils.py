...
def save_weights_var(acas, variancenets , decoders, model_name, run_date):
    if not os.path.exists(WEIGHT_DIR):
        os.makedirs(WEIGHT_DIR)
    state = {'aca_state_dict': [aca.state_dict() for aca in acas],
        'variancenet_state_dict': [variancenet.state_dict() for variancenet in variancenets],
...
def load_weights_var(acas, variancenets, decoders, filename):
    path = os.path.join(filename)
    state = torch.load(path)
    for aca, aca_state in zip(acas, state['aca_state_dict']):
        aca.load_state_dict(aca_state, strict=False)
    for variancenet, variancenet_state in zip(variancenets, state['variancenet_state_dict']):
        variancenet.load_state_dict(variancenet_state, strict=False)
    for decoder, decoder_state in zip(decoders, state['decoder_state_dict']):
