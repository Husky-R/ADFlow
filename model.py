...
def load_decoder_arch(c, dim_in):
    if   c.dec_arch == 'daflow':
        decoder = freia_cflow_head(c, dim_in)
    else:
        raise NotImplementedError('{} is not supported NF!'.format(c.dec_arch))
    return decoder
def load_maca(dim_in):
    aca = ACA(dim_in, dim_in)
    return aca

def load_VarianceNet(input_dim, num_components = 10):
    variancenet = VarianceNet(input_dim, num_components)
    return variancenet
activation = {}
...
class SNet(nn.Module):
    ...
    def __init__(self, channels=3, num_of_layers=5, patch_size=3, input_size=224):
        ...
        # 新增池化层，patch大小为patch_size x patch_size
        self.pool = nn.AdaptiveAvgPool2d((input_size//patch_size, input_size//patch_size))
